...
get_nws_forecast_info <- function() {
  ...
  forecast_data_tibble <- as_tibble(forecast_flattened$properties$periods) %>%
    mutate(
      startTime = ymd_hms(startTime, tz = "America/New_York"),
...
tides <- get_tide_info_tibble(date) %>%
  mutate(dttm = ymd_hm(t, tz = "America/New_York"), height = as.numeric(v))

tides_hilo <- get_tide_hilo_tibble(date) %>%
  mutate(dttm = ymd_hm(t, tz = "America/New_York"), height = as.numeric(v))

weather_forecast <- get_nws_forecast_info() %>%
  filter(startTime <= first_light_hms, endTime > first_light_hms)

annotation_text <- tribble(
  ~label_text,
  paste0(
    "First Light @ ",
    format(first_light, "%H:%M"),
    "<br>",
    "Temp: ",
    ...
    " ",
    weather_forecast$windDirection,
    "<br>",
    "Sky: ",
    weather_forecast$shortForecast,
    "<br>",
    "Precip %: ",
    weather_forecast$probabilityOfPrecipitation.value
  )
)
...
p1 <- tides %>%
  ...
  geom_point(data = tides_hilo, aes(y = height), size = 2) +
  annotate(
    "rect",
  ...
  geom_image(
    data = tibble(dttm = first_light_hms + minutes(45), height = 0.5),
    aes(image = "walk_icon.png", x = dttm, y = max(tides$height) * 1.2),
    size = 0.15
  ) +
  scale_x_datetime(
    date_labels = "%H:%M\n%d %b",
...
annotation_text_dark <- tribble(
  ~label_text,
  paste0(
    color_text(
      paste0("First Light @ ", format(first_light, "%H:%M")),
      "#FF8C00"
    ),
    "<br>",
    "Temp: ",
...
p1_dark <- tides %>%
  ...
  geom_point(data = tides_hilo, aes(y = height), color = "white", size = 2) +
  annotate(
    "rect",
    xmin = first_light_hms,
    xmax = first_light_hms + minutes(90),
  ...
  geom_image(
    data = tibble(dttm = first_light_hms + minutes(45), height = 0.5),
    aes(image = "walk_icon_white.png", x = dttm, y = max(tides$height) * 1.4),
    size = 0.12
  )

ggsave("beach_walk_dark.png", plot = p1_dark, width = 7, height = 7)
